...
function preventformSubmit() {//3a
  function handleFormSubmit(event) {//6a
...
function activateInput() {//9a - e tornar-lo reativo. fazer com que a gente escute a digitação dele
  function insertName(newName) { //14a - e chamr o parametro de nome que quiser
  ...
  function updateName(newName) { //51a
    globalNames[currentIndex] = newName; //60a
  }

  function handleTyping(event) { //11a - estou declarando a função. So pegar quando o caracter for ENTER. Quando um evento dispara uma função. Ele manda algo implicitamente que a gente captura com EVENT
...
function render() { //14a - 
  function createDeleteButton(index) { //32a - inves de colocar "i" nomeia como index
    function deleteName() { //37a
//isso se chama de clousure
  ...
  function createSpan(name, index) { //57a
    function editItem() { //44a
  ...
  for (var i = 0; i < globalNames.length; i++) {//18a para pegar o tamamho do vetor.Vai percorrer de 0 ATE o tamanho do vetor. Eu quero de 0 a 3
...
function clearInput() { //23a  - para excluir apos digitar algo no formulario
  inputName.value = '';//24a
  inputName.focus(); //25a
}
